...
export function ProjectTerminal({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: Project identity owns this terminal instance.
  useEffect(() => {
    let cancelled = false;
  ...
  useEffect(() => {
    ...
    const frame = requestAnimationFrame(() => {
      const size = fitTerminal(terminalRef.current, fitAddonRef.current);
      if (size) {
        connectionRef.current?.resize(size.cols, size.rows);
      }
  ...
  const connect = () => {
    ...
    if (!terminal || !fitAddon || hasActiveRun || phase === "connecting" || phase === "ready") {
      return;
    }
    ...
    try {
      connectionRef.current = connectProjectTerminal(projectId, size, {
        onClose() {
        ...
        onExit(exitCode) {
          ...
          setPhase("closing");
          if (exitCode !== 0) {
            setError(new Error(`终端进程已退出（code ${exitCode}）。`));
          }
        },
  ...
  return (
    <section className="project-terminal-view" hidden={!active}>
      <div className="project-terminal-toolbar">
        <div
          aria-live="polite"
          className={`project-terminal-status terminal-status-${phase}`}
          title={
            expiresAt ? `Session expires ${new Date(expiresAt).toLocaleTimeString()}` : undefined
          }
        >
          <span aria-hidden="true" />
          {terminalPhaseLabel(phase)}
        </div>
        {phase === "ready" || phase === "connecting" || phase === "closing" ? (
          <button
      ...
      <div className="project-terminal-canvas" ref={containerRef} />
    </section>
  );
}

function fitTerminal(terminal: XTerm | null, fitAddon: FitAddon | null) {
