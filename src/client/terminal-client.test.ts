...
describe("Terminal browser protocol", () => {
  it("accepts only public normalized server messages", () => {
    expect(
      parseTerminalServerMessage(
        '{"type":"ready","expiresAt":"2026-07-26T08:30:00.000Z"}',
      ),
    ).toEqual({
    ...
    expect(
      parseTerminalServerMessage(
        '{"type":"closed","exitCode":0}',
      ),
    ).toEqual({ exitCode: 0, type: "closed" });
    expect(
      parseTerminalServerMessage(
        '{"type":"error","code":"project_busy"}',
      ),
    ).toEqual({ code: "project_busy", type: "error" });
  });
  ...
  it("rejects malformed or provider-specific payloads", () => {
    ...
    expect(
      parseTerminalServerMessage(
        '{"type":"ready","providerSandboxId":"private"}',
      ),
    ).toBeNull();
    expect(
      parseTerminalServerMessage(
