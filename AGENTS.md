...
- `SandboxRuntime` owns sandbox lifecycle and generic process execution. `AgentRuntime` owns an individual Agent protocol and normalized events. Do not merge those responsibilities.
- Pi is the only registered AgentRuntime. Do not expose or implement Goose, Claude Code, or Codex CLI merely because their IDs are reserved.
