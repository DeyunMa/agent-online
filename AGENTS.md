...
- V1 stores product state in D1: authentication, Project metadata, user-visible messages, `AgentRun` lifecycle, and per-run aggregate usage. The sandbox filesystem is the only workspace copy; a stopped or expired sandbox may lose its files.
- `AgentRun` is one short-lived Agent execution, normally one user turn. A Project has at most one non-terminal AgentRun. Do not reintroduce durable Agent sessions, workspace revision history, or raw transcript retention without a new ADR.
