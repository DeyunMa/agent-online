...
- The E2B image keeps platform tools under read-only `/opt/agent-online` and user code under writable `/workspace`. Preview must use the pinned platform Vite binary, require a regular `/workspace/index.html`, classify missing declared dependencies before claiming a session, and never use `npx`, a Project script, a Project Vite config, or a browser-provided command.
- Pi is the default and currently validated AgentRuntime. Additional runtimes require an accepted ADR, a dedicated adapter, an explicit server allowlist or feature flag, ModelGateway compatibility, cancellation/final-output/usage tests, and a real end-to-end validation before UI exposure. Pi is the only installed runtime; ADR-0014 retires the former second runtime. Reserved IDs alone never imply support.
...
- V1 stores product state in D1: authentication, Project metadata, user-visible messages, `AgentRun` lifecycle, per-run aggregate usage, private model admission counts/in-flight locks and one current user admission-counter row under ADR-0012, a minimal per-Run usage archive for deleted Projects, and only the current ephemeral Terminal/Preview coordination records. The archive must not contain Messages, prompts, replies, failure details, files, or Provider references. Never persist Terminal input/output, Preview content/history, or Git status/diff/history. Changes is a bounded read of the current sandbox working tree/index and cannot be attributed to one Run; it must reject extra repository config scopes and explicitly mark unsupported paths instead of reporting clean. The sandbox filesystem is the only workspace copy; a stopped or expired sandbox may lose its files.
