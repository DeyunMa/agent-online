...
这个 opt-in E2E 测试验证一条刻意收窄的真实路径：本机 Worker 兼容的 `ModelGateway` -> Cloudflare Quick Tunnel -> 带预装 Pi 的 E2B 临时沙箱 -> Pi 无 session、调用文件工具 -> Gemini 3.6 Flash 最终回复与实际 usage。
...
- 网关只接受 `POST /v1/chat/completions`，capability 绑定一个固定模型和输出 token 上限；Gemini Key 由网关替换，Pi 的消息与函数工具通过 Gemini 官方 OpenAI-compatible endpoint 转发。
...
2. Pi 调用工具写入 `/workspace/agent-online-e2e.txt`，文件内容和最终回复都包含 `AGENT_ONLINE_E2E_OK` 标记。
...
通过这个 spike 仅证明 E2B、Pi custom provider、Gemini 协议转换和密钥边界能够共同工作。它本身不验证 Cloudflare 远程 Workflow、D1 CAS、跨请求取消、空闲 TTL、页面恢复或 Free 计划 CPU 限额；这些产品路径已由私有 Preview 单独完成代表性验收。正式执行实现仍以 [ADR-0003](../adr/0003-agent-run-workflow.md) 为准。
